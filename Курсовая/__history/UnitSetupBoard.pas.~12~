...
uses UnitMainForm, UnitCreatingFigures, UnitTimer, UnitMyMessageBoxes;
...
begin
    ...
    begin
        if not GameIsSaved then
            ...
            #10#13 + 'Иначе текущая партия будет удалена.', True) then
                NSaveAsClick(FormMain)
            else
                SaidNoSaving := True;

        if GameIsSaved or SaidNoSaving or MyMessageBoxYesNo('Начать новую партию?', 'Вы уверены, что хотите начать новую партию?' + #10#13 +
            'Текущая партия будет удалена.', True) then
        begin
            ResetChessboardToStandart();
            PanelGameEnded.Visible := False;
        end;
